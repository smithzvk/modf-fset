  ...
  (let ((seq (fset:seq 'a 'b 'c 'd)))
    ...
    (is (fset:equal? (fset:seq 'a 1 2 'd)
                     (modf (fset:subseq seq 1 3)
                           (fset:seq 1 2) )))
    (is (fset:equal? (fset:seq 1 'b 'c 'd)
                     (modf (fset:first seq)
                           1 )))
    (is (fset:equal? (fset:seq 'a 'b 'c 1)
                     (modf (fset:last seq)
                           1 )))
    (is (fset:equal? (fset:seq 'a 1 2)
                     (modf (fset:less-first seq)
                           (fset:seq 1 2) )))
    (is (fset:equal? (fset:seq 1 2 'd)
                     (modf (fset:less-last seq)
                           (fset:seq 1 2) )))
    (is (fset:equal? (fset:seq 'a 1 2)
                     (modf (fset:tail seq)
                           (fset:seq 1 2) )))))
